...
try {
  ...
  for (const line of lines) {
    ...
    score += extrapolateHistory(input)
  }
...
export function beforeSequence(row1: number[], row2: number[]): number[] {
  let result: number[] = row1

  const next = row1[0] - row2[0]

  result.unshift(next)
  return result
}

export function extrapolateBackwardsHistory(input: number[]): number {
  let rows: number[][] = []
  rows.push(input)
  ...
  for (let i = 1; i < rows.length; i++) {
    rows[i] = beforeSequence(rows[i], rows[i - 1])
  }

  let firstElement = rows[rows.length - 1][0]
  return firstElement
}
