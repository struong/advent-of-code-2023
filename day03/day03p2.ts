...
try {
  ...
  class IndexRange {
    ...
    gearSum(others: IndexRange[]) {
      let numberTouching = 0
      let sum = 1
      for (const other of others) {
        if (this.isTouching(other)) {
          numberTouching++
          sum *= parseInt(other.value)
        }
      }

      return numberTouching === 2 ? sum : 0
    }
  }
  ...
  for (const line of lines) {
    ...
    const symbolMatches = Array.from(line.matchAll(/\*/g))
  ...
  symbolRanges.map((symbolRange) => {
    sum += symbolRange.gearSum(numberRanges)
  })
